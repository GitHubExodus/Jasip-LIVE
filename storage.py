...
# ==========================================
# ========= Generic Parquet Read & Write ===
# ==========================================

def read_parquet_from_r2(r2_manager: R2Manager, key: str) -> Optional[pd.DataFrame]:
    """Downloads Parquet bytes from R2, with local disk cache fallback."""
...
def write_parquet_to_r2(r2_manager: R2Manager, df: pd.DataFrame, key: str) -> None:
    """Serializes DataFrame to Parquet, writes local cache, and uploads to R2."""
...
def _df_to_equity_data(symbol: str, df: Optional[pd.DataFrame]) -> Any:
    """Reconstructs EquityData using the explicit symbol parameter."""
    try:
        from equity import EquityData
        if df is None or df.empty:
            return EquityData(symbol=symbol, records=[])
        if hasattr(EquityData, "from_dataframe"):
            return EquityData.from_dataframe(symbol, df)
        return EquityData(symbol=symbol, records=[])
    except Exception as e:
        logger.error(f"Error converting DataFrame to EquityData for {symbol}: {e}")
        from equity import EquityData
        return EquityData(symbol=symbol, records=[])


# ==========================================
# ========= Stock & Contribution Storage ===
# ==========================================

def load_stock_equity(r2_manager: R2Manager, symbol: str) -> Optional[pd.DataFrame]:
    prefix = getattr(config, "R2_EQUITY_PREFIX", "equity_test")
...
class StorageEngine:
    """Public storage API encapsulating R2 operations."""
...
# Thread-Safe Instances & Top-Level Functions
...
def load_equity(symbol: str) -> Any:
    ...
    return _df_to_equity_data(symbol, df)
